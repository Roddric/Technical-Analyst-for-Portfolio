...
- Tests for the whole pipeline live in the parent's `tests/` (run `pytest tests -q` from the parent); there are no tests specific to this directory. Note when verifying against `results/portfolio_backtest_2024_2026.md`: the advisor reports **target** weights, while the report's holdings are the smoothed book (each rebalance blends halfway from the previous book toward that target), so the advisor's signals/ranks/hold set should match a rebalance row but the weight numbers will differ by the blend.
